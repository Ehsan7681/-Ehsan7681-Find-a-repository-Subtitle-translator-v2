document.addEventListener('DOMContentLoaded', () => {
    ...
    // کنترل‌های تنظیم سایز
    const textareaHeightSlider = document.getElementById('textarea-height-slider');
    const textareaHeightValue = document.getElementById('textarea-height-value');
    ...
    // رویدادهای کنترل سایز
    if (textareaHeightSlider) {
        textareaHeightSlider.addEventListener('input', updateTextareaHeight);
    }
    if (panelWidthSlider) {
    ...
    function loadSavedSettings() {
        ...
        // بارگذاری تنظیمات سایز
        if (window.innerWidth <= 768) {
            const savedTextareaHeight = localStorage.getItem('textarea_height') || 100;
            const savedPanelWidth = localStorage.getItem('panel_width') || 100;
            const savedFontSize = localStorage.getItem('font_size') || 14;
            
            if (textareaHeightSlider) {
                textareaHeightSlider.value = savedTextareaHeight;
                textareaHeightValue.textContent = `${savedTextareaHeight}px`;
            ...
            if (panelWidthSlider) {
                panelWidthSlider.value = savedPanelWidth;
                panelWidthValue.textContent = `${savedPanelWidth}%`;
                const container = document.querySelector('.container');
                container.style.width = `${savedPanelWidth}%`;
            }
    ...
    function updateTextareaHeight() {
        const height = textareaHeightSlider.value;
        const textareas = document.querySelectorAll('textarea');
        textareas.forEach(textarea => {
            textarea.style.height = `${height}px`;
        });
        textareaHeightValue.textContent = `${height}px`;
        localStorage.setItem('textarea_height', height);
    }
    ...
    function updatePanelWidth() {
        ...
        panelWidthValue.textContent = `${width}%`;
        localStorage.setItem('panel_width', width);
    }
